...
class ExportManifest:
  ...
  def add_file(
    self, record_type: str, file_path: Path, record_count: int, file_size: int
  ):
  ...
  def add_cleaning_stats(self, cleaning_stats: dict[str, Any]):
    """Add data cleaning statistics to manifest."""
    self.manifest_data["cleaning_stats"] = cleaning_stats
...
class DataExporter:
  ...
  def _records_to_dataframe(self, records: Sequence[AnyRecord]) -> pd.DataFrame:
    ...
    # Remove redundant columns (type vs record_type, unit vs record_unit)
    columns_to_remove = []
    if "record_type" in df.columns and "type" in df.columns:
      # Keep "type" as it's more readable, remove "record_type"
      columns_to_remove.append("record_type")
    if "record_unit" in df.columns and "unit" in df.columns:
      # Keep "unit" as it's more readable, remove "record_unit"
  ...
  def _apply_data_cleaning(
    self, records_by_type: dict[str, list[AnyRecord]]
  ) -> dict[str, list[AnyRecord]]:
    """Apply data cleaning to records by type.

    Args:
        records_by_type: Dictionary of records grouped by type

    Returns:
        Dictionary of cleaned records grouped by type
    """
    ...
    for record_type, records in records_by_type.items():
      ...
      cleaning_stats["total_original_records"] += len(records)  # type: ignore
      cleaning_stats["total_cleaned_records"] += len(cleaned_records)  # type: ignore
      cleaning_stats["total_duplicates_removed"] += duplicates_removed  # type: ignore
      cleaning_stats["records_by_type"][record_type] = {  # type: ignore
        "original_count": len(records),
        "cleaned_count": len(cleaned_records),
        "duplicates_removed": duplicates_removed,
      }

    # Add cleaning stats to manifest
